...
- **Generic Studio-output engine**: `generate_studio_output` / `get_studio_output_status` / `download_studio_output` / `get_studio_output_content` cover all 9 NotebookLM Studio output types by schema. 8 of 9 (`audio`, `video`, `infographic`, `slides`, `mindmap`, `datatable`, `quiz`, `flashcards`) are wired to live-verified strategies; only `report` still returns a clear "not yet implemented (Phase 2)" error. `generate_audio`/`get_audio_status`/`download_audio` remain as backward-compatible aliases for `output_type: "audio"`.
