...
import { CONFIG, NOTEBOOKLM_AUTH_URL, isNotebookAppUrl } from "../config.js";
...
export class AuthManager {
   ...
   *
   * SIMPLE & RELIABLE: Just wait for URL to change to the NotebookLM app
   * (notebook.google.com, or the legacy notebooklm.google.com redirect —
   * see isNotebookAppUrl in config.ts).
   */
   ...
   *
   * Just checks if URL changes to the NotebookLM app - no complex UI element searching!
   ...
   *
   * Just checks if URL is on the NotebookLM app - no complex UI element searching!
   * Matches the simplified approach used in performLogin().
   */
