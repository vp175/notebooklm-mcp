...
- **Task 6:** engine + audio retrofit are built in full. The tile-discrimination selector fix uses a grounded fallback chain — the new icon-scoped selector is tried first, with the existing (currently-working, single-tile-type-safe) broad selector kept as a trailing fallback — so behavior cannot regress even if the icon-redisplay hypothesis turns out wrong once it's actually checked against the live site. The mandatory live-account regression check (original Step 7) is **deferred**, not skipped-and-claimed-done.
